    ...
    <div class="navbar-header">
        ...
        @if (isset($config->config['layout']['logo']))
            <a href="{{ route('dashboard') }}" class="navbar-brand"><img
                    src="{{ route('imagem.render', 'company/p/' . $config->config['layout']['logo']) }}" alt=""
                    class="img-fluid mr-2"><b>{{ $config->nome ?? 'Pitter' }}</b></a>
        @else
            <a href="{{ route('dashboard') }}" class="navbar-brand"><span
                    class="navbar-logo"></span><b>{{ $config->nome ?? 'Pitter' }}</b></a>
        @endif
        @if ($headerMegaMenu)
    ...
    <ul class="navbar-nav navbar-right">
        ...
        <li class="dropdown navbar-user">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                <x-jet-dropdown-link href="{{ route('controle.profile.edit') }}" class="dropdown-item">Perfil
                </x-jet-dropdown-link>
                <!-- Authentication -->
